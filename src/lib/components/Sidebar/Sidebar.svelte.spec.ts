...
describe('Sidebar', async () => {
    ...
    describe('rendering', async () => {
        ...
        it('should set width css variables and current width', async () => {
            const { container } = await render(Sidebar, { items, width: 300, collapsedWidth: 72 })
            const root = container.firstElementChild as HTMLElement
    ...
    describe('items', async () => {
        ...
        it('should forward menu props to the NavigationMenu', async () => {
            const { container } = await render(Sidebar, { items, menu: { variant: 'link' } })
    ...
    describe('collapse', async () => {
        ...
        it('should render the footer toggle when toggle is true', async () => {
            const { container } = await render(Sidebar, { items, toggle: true })
        ...
        it('should reflect an external collapsed prop', async () => {
            const { container } = await render(Sidebar, { items, collapsed: true })
            const root = container.firstElementChild as HTMLElement
            expect(root.dataset.collapsed).toBe('true')
            expect(root.style.width).toBe('64px')
        })
        ...
        it('should not be inert while expanded', async () => {
            const { container } = await render(Sidebar, { items, collapsible: 'offcanvas' })
            const root = container.firstElementChild as HTMLElement
    ...
    describe('variant', async () => {
        ...
        it('should expose the floating variant', async () => {
            const { container } = await render(Sidebar, { items, variant: 'floating' })
            const root = container.firstElementChild as HTMLElement
    ...
    describe('header', async () => {
        ...
        it('should render a close button that collapses on click', async () => {
            const { container } = await render(Sidebar, { items, title: 'Acme', close: true })
            const root = container.firstElementChild as HTMLElement
    ...
    describe('rail', async () => {
        ...
        it('should render an interactive rail that toggles collapse', async () => {
            const { container } = await render(Sidebar, { items, rail: true })
            const root = container.firstElementChild as HTMLElement
