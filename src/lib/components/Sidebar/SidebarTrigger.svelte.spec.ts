...
describe('SidebarTrigger', async () => {
    it('should render a button', async () => {
        const { container } = await render(SidebarTrigger, {})
    ...
    it('should forward a custom label', async () => {
        const { container } = await render(SidebarTrigger, { label: 'Menu' })
        expect(container.textContent).toContain('Menu')
    })

    it('should forward button props such as color and variant', async () => {
        const { container } = await render(SidebarTrigger, { color: 'primary', variant: 'solid' })
        expect(container.querySelector('button')).not.toBeNull()
    })

    it('should expose aria-expanded from the collapsed prop', async () => {
        const { container } = await render(SidebarTrigger, { collapsed: false })
    ...
    it('should delegate to the sidebar api when given one', async () => {
        ...
        const { container } = await render(SidebarTrigger, { api })
