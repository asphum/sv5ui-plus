...
describe('Tree', async () => {
    ...
    const renderWithApi = async (props: Record<string, unknown>) => {
    ...
    describe('rendering', async () => {
        it('renders a role=tree root', async () => {
            const { container } = await render(Tree, { items: sampleItems })
            expect(getTree(container).getAttribute('role')).toBe('tree')
        })

        it('renders only top-level items when collapsed', async () => {
            const { container } = await render(Tree, { items: sampleItems })
            expect(getItems(container).length).toBe(4)
        })

        it('expands nodes listed in defaultExpanded', async () => {
        ...
        it('expands nodes marked item.defaultExpanded', async () => {
            const { container } = await render(Tree, { items: checkboxItems })
            expect(getItems(container).length).toBe(3)
        })

        it('renders labels via custom labelKey', async () => {
            interface NamedItem extends TreeItem {
                name: string
            }
            const items: NamedItem[] = [{ name: 'Custom Node' }]
            const { container } = await render(Tree, { items, labelKey: 'name' } as never)
            expect(container.textContent).toContain('Custom Node')
        })

        it('treats leaves and empty children as non-expandable', async () => {
            const { container } = await render(Tree, {
        ...
        it('sets aria-level, aria-setsize and aria-posinset on nested items', async () => {
            const { container } = await render(Tree, {
                items: sampleItems,
        ...
        it('sets aria-multiselectable only when multiple', async () => {
            const single = await render(Tree, { items: sampleItems })
            expect(getTree(single.container).hasAttribute('aria-multiselectable')).toBe(false)
            const multi = await render(Tree, { items: sampleItems, multiple: true })
            expect(getTree(multi.container).getAttribute('aria-multiselectable')).toBe('true')
        })

        it('resolves duplicate keys to index paths', async () => {
            const { container } = await render(Tree, {
        ...
        it('keeps keys unique when the index-path fallback also collides', async () => {
            const { container } = await render(Tree, {
    ...
    describe('expansion', async () => {
        ...
        it('fires onExpandedChange with the new keys', async () => {
            ...
            const { container } = await render(Tree, { items: sampleItems, onExpandedChange })
            await getLink(container, 'src')!.click()
            await vi.waitFor(() => {
        ...
        it('controlled expanded=[] overrides item.defaultExpanded', async () => {
            const { container } = await render(Tree, { items: checkboxItems, expanded: [] })
        ...
        it('calls item.onToggle with the new state', async () => {
            ...
            const { container } = await render(Tree, { items })
        ...
        it('does not expand or select disabled parents', async () => {
            ...
            const { container } = await render(Tree, { items: sampleItems, onValueChange })
        ...
        it('does not expand disabled parents via keyboard or api', async () => {
            const { container, getApi } = await renderWithApi({ items: sampleItems })
        ...
        it('ignores defaultExpanded on disabled parents', async () => {
            const { container } = await render(Tree, {
                items: [
                    {
                        label: 'locked',
    ...
    describe('api', async () => {
        it('select, deselect and clearSelection update the value', async () => {
            const onValueChange = vi.fn()
            const { getApi } = await renderWithApi({
                items: sampleItems,
                multiple: true,
                onValueChange
            })
    ...
    describe('styling', async () => {
        it('applies ui overrides and item classes', async () => {
            const items: TreeItem[] = [
